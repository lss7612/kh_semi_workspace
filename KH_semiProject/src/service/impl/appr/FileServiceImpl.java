...
import service.face.appr.FileService;
...
//					System.out.println(endDay);
					...
					appr.setMid_auth( Integer.parseInt(item.getString()));
					
				}  else if("secondApprovalNo".equals(key)) {
					//----파이널결재자
					if(item.getString()!=null) {
					System.out.println("secondApprovalNo = " + item.getString());
					appr.setFinal_auth( Integer.parseInt(item.getString()));
					}else {
						
						appr.setFinal_auth(0);
					}
				}
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
